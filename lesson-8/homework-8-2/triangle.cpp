...
#include <string>
    ...
    {
       // if (get_sides_number() != correct_sides_num)
       // {
       //     throw FiguresException("неверное количество сторон"); //эта ошибка невозможна, так как количество сторон 3 определено в конструкторе
       // }

        if ((A+B+C) != correct_angles_sum)
        {
            throw FiguresException("сумма углов не равна " + std::to_string(correct_angles_sum));
        }
